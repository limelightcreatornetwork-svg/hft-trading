...
class TradingSystem:
    ...
    def _create_kalshi_client(self) -> KalshiClient:
        """Create Kalshi client from environment."""
        prefix = "KALSHI_DEMO_" if self.paper else "KALSHI_"
        ...
        config = KalshiConfig(
            ...
            password=password,
            api_key=api_key,
            environment=KalshiEnvironment.DEMO if self.paper else KalshiEnvironment.PRODUCTION,
        )
    ...
    async def test_connections(self):
        ...
        if self.kalshi_client:
            try:
                ...
                markets = await self.kalshi_client.get_markets(limit=1)
                print(f"  ✓ Markets: {len(markets.get('markets', []))} available")
